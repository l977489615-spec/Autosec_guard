...
**智驭安盾（SmartDrive Shield Edge）** 是一款运行在测试人员本机、实验室工控机或车端测试工作站上的智能网联汽车（ICV）漏洞验证产品。<br>
它的产品定位不是“云端下发、车端执行”的联动平台，而是 **本机安装、本机连接车辆/台架、本机执行 PoC、本机沉淀证据、本机生成报告** 的边缘端安全验证工作站。
...
- 不交付 `client/src`
- 不交付 `server/*.py`
- 不交付 `server/pocs` 源码目录
- 前端资源和内置 PoC 随可执行文件打包
- 数据库与日志写入 `AUTOSEC_DATA_DIR` 或系统用户数据目录
...
不是。当前主产品路径是本地边缘端工作站。旧的远端节点、任务队列和相关接口已经移除，默认 UI 和扫描流程只依赖本机执行引擎。
