...
import { executePocScript, checkBackendHealth, getBackendUrl } from '../services/api';
...
const ManualTestModal: React.FC<ManualTestModalProps> = ({ poc, isOpen, onClose, globalConnection }) => {
  ...
  const handleRun = async () => {
    ...
    setIsRunning(true);
    setTestResult('idle');
    setConsoleOutput(p => [...p, `[*] Generating script payload for ${poc.name}...`]);

    try {
        // Dynamic Script Generation
        let scriptContent = "";
        if (poc.scriptGenerator) {
             scriptContent = poc.scriptGenerator(localParams);
        } else {
             scriptContent = poc.codeSnippet; // Fallback to static
        }

        setConsoleOutput(p => [...p, `[*] Sending ${scriptContent.length} bytes to execution engine...`]);
            ...
            <div className="w-full md:w-1/3 bg-cyber-800 p-4 border-r border-cyber-700 space-y-4 overflow-y-auto">
                ...
                <div className="pt-4">
                    <button 
                        ...
                        className={`w-full py-2 flex items-center justify-center gap-2 rounded font-bold text-sm transition-all ${
                            isRunning || !backendOnline ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-cyber-danger text-white hover:bg-red-600 shadow-lg'
                        }`}
                    >
