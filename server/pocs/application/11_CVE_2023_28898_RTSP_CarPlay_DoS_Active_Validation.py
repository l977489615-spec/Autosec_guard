...
Usage: python3 11_CVE_2023_28898_RTSP_CarPlay_DoS_Active_Validation.py <args>
"""
...
VULN = {
    ...
    "component":      "Unknown",
    "type":           "Unknown",
    "summary":        "RTSP CarPlay DoS",
    "source_url":     "https://nvd.nist.gov/vuln/detail/CVE-2023-28898",
    "affected":       [{"vendor": "Unknown", "product": "Unknown", "versions": []}],
    "references":     ["https://nvd.nist.gov/vuln/detail/CVE-2023-28898"],
    "signature_tokens": ["CVE-2023-28898"],
}


def _run_poc(plugin, vuln=None) -> dict:
    """
    CVE-2023-28898 主动探针包装（兼容旧式 exploit() 实现）。
    ...
    evidence = {
        "cve":       vuln.get("cve", "CVE-2023-28898") if vuln else "CVE-2023-28898",
...
class RTSPCarPlayDoSPlugin(IVIVulnerabilityPlugin):
    ...
    def _send_rtsp(self, host, port, request, timeout=None):
        ...
        try:
            ...
            sock.close()
            # If server closes connection immediately, return None to indicate abnormal behavior
            if not response:
                return None
            return response
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 11_CVE_2023_28898_RTSP_CarPlay_DoS_Active_Validation.py <args>")
        sys.exit(1)
