...
# -*- coding: utf-8 -*-
"""
detect_setAllowFileAccess_vuln.py

Detect WebView setAllowFileAccess(true) Vulnerability in CarSetting.apk
Vulnerability Description:
WebSettings.setAllowFileAccess(true) 启用 WebView 对本地文件系统的访问权限（默认在 Android 11+ 为 false）。
若 WebView 同时加载外部网页（HTTP/HTTPS），攻击者可通过恶意 JS 读取本地敏感文件（如 /etc/passwd、应用私有数据），
导致敏感信息泄露；若结合文件上传/下载功能，还可能篡改本地文件。

Key Features:
1. Build JSON-format API-Class map (saved as APIMap_CarSetting_setAllowFileAccess.json)
2. Detect setAllowFileAccess(true) calls (core vulnerable configuration)
...
"""
POC_TAG = "28. 检测设备app是否使用了不安全的Webview文件访问接口..."
...
JSON_OUTPUT_PATH = os.path.join(os.getcwd(), "APIMap_CarSetting_setAllowFileAccess.json")

# ==========================
# Configuration: Target API (setAllowFileAccess(true))
# 核心API：WebSettings.setAllowFileAccess(boolean allow)
# 方法签名：Landroid/webkit/WebSettings;->setAllowFileAccess(Z)V
# 漏洞点：参数为 true（启用本地文件访问），false 为安全配置
# ==========================
TARGET_APIS = {
    "WebSettings.setAllowFileAccess(true)": {
        "smali_patterns": [
            # 精确匹配完整invoke指令（参数为 true → 对应 smali 中的 0x1）
            re.compile(
                r'invoke-virtual\s*\{\s*.+?,\s*0x1\s*\}\s*Landroid/webkit/WebSettings;->setAllowFileAccess\(Z\)V',
                re.IGNORECASE | re.DOTALL
            ),
            # 匹配参数为 true 的简化格式（0x1 可能带空格）
            re.compile(
                r'Landroid/webkit/WebSettings;->setAllowFileAccess\(Z\)V\s*.+?0x1',
                re.IGNORECASE | re.DOTALL
            ),
            # 匹配方法调用 + true 参数（兼容不同参数位置写法）
            re.compile(
                r'setAllowFileAccess\(Z\)V.*?0x1|0x1.*?setAllowFileAccess\(Z\)V',
                re.IGNORECASE | re.DOTALL
            )
        ],
        "full_method": "Landroid/webkit/WebSettings;->setAllowFileAccess(Z)V",
        "vulnerable_param": "true (0x1 in smali)"
    }
}
...
def save_api_class_map(api_class_map: ApiClassMap) -> bool:
    ...
    try:
        json_content = {
            "description": "API-Class map for setAllowFileAccess(true) vulnerability detection",
...
def analyze_vulnerability(api_class_map: ApiClassMap) -> Tuple[bool, Dict[str, List[Dict[str, Union[str, List[int]]]]]]:
    """Detect vulnerability by checking setAllowFileAccess(true) calls"""
...
def process_device(device: str) -> bool:
    ...
    if is_vulnerable:
        logging.error("[VULNERABLE] WebSettings.setAllowFileAccess(true) detected!")
        ...
        logging.error(f"\nRisk Explanation:")
        logging.error(f"1. setAllowFileAccess(true) enables WebView to access local file system (files, directories)")
        logging.error(f"2. Critical risk scenario: WebView loads external untrusted web pages (HTTP/HTTPS)")
        logging.error(
            f"   - Attackers can inject malicious JavaScript to read local sensitive files (e.g., /etc/passwd, app private data, user credentials)")
        logging.error(f"   - May allow file upload/download tampering, leading to malware installation")
        logging.error(f"3. Android version note:")
        logging.error(f"   - Android 11 (API 30+) default: setAllowFileAccess(false) (safer)")
        logging.error(f"   - Android < 11 default: setAllowFileAccess(true) (legacy risk)")
        logging.error(
            f"4. Even if loading local files: Enabling this increases attack surface if local files are compromised")

        logging.error(f"\nFix Recommendation:")
        logging.error(f"1. Disable local file access: Set setAllowFileAccess(false) (core fix)")
        logging.error(f"   - Code example: webView.getSettings().setAllowFileAccess(false);")
        logging.error(f"2. Restrict WebView content sources:")
        logging.error(f"   - Only load trusted local assets (file:///android_asset/) or internal web pages")
        logging.error(f"   - Avoid loading external web pages (HTTP/HTTPS) in WebView with local file access enabled")
        logging.error(f"3. Additional hardening:")
        logging.error(
            f"   - Disable setAllowFileAccessFromFileURLs(false) (prevents file URLs from accessing other files)")
        logging.error(f"   - Disable setAllowUniversalAccessFromFileURLs(false) (blocks cross-origin file access)")
        logging.error(f"   - Enable WebView Safe Browsing (webView.getSettings().setSafeBrowsingEnabled(true))")
        logging.error(f"4. For Android < 11: Explicitly set setAllowFileAccess(false) (overrides default)")
        logging.error(f"5. Avoid mixing local file access and external content: Never enable both simultaneously")
        return True
    else:
        logging.info("[SAFE] No setAllowFileAccess(true) calls detected")
        logging.info(f"Target API: {list(TARGET_APIS.keys())[0]}")
        logging.info(f"Note: Default behavior depends on Android version (Android 11+ default is false)")
        return False


def run_check():
    parser = argparse.ArgumentParser(
        description="Detect WebView setAllowFileAccess(true) Vulnerability (auto-save API map)")
