...
# -*- coding: utf-8 -*-
"""
detect_openFileOutput_world_readable_vuln.py

Detect openFileOutput(..., MODE_WORLD_READABLE) Vulnerability in CarSetting.apk
Vulnerability Description:
Context.openFileOutput(String name, int mode) 用于创建/写入应用私有文件。
若第二个参数包含 MODE_WORLD_READABLE（值为 0x01），表示文件对设备上所有应用可读，
攻击者可通过其他应用读取该文件中的敏感数据（如用户凭证、配置信息、隐私数据），导致信息泄露。

Key Features:
1. Build JSON-format API-Class map (saved as APIMap_CarSetting_openFileOutput.json)
2. Detect openFileOutput calls with MODE_WORLD_READABLE (core vulnerable configuration)
...
"""
POC_TAG = "29. 检测设备app是否使用了不安全的文件全局可读写访问接口..."
...
JSON_OUTPUT_PATH = os.path.join(os.getcwd(), "APIMap_CarSetting_openFileOutput.json")

# ==========================
# Configuration: Target API (openFileOutput + MODE_WORLD_READABLE)
...
# 注意：mode 支持组合（如 MODE_PRIVATE | MODE_WORLD_READABLE = 0x01），只要包含 0x01 即漏洞
# ==========================
TARGET_APIS = {
    "Context.openFileOutput(..., MODE_WORLD_READABLE)": {
        "smali_patterns": [
            # 1. 精确匹配 mode = MODE_WORLD_READABLE（单独使用，值 0x1）
            re.compile(
                r'invoke-virtual\s*\{\s*.+?,\s*.+?,\s*0x1\s*\}\s*Landroid/content/Context;->openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;',
                re.IGNORECASE | re.DOTALL
            ),
            # 2. 匹配 mode 组合（如 0x1 | 0x2 = 0x3，只要包含 0x1 即匹配）
            re.compile(
                r'Landroid/content/Context;->openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;\s*.+?(0x[13579bdf])',
                re.IGNORECASE | re.DOTALL
            ),
            # 3. 模糊匹配：方法签名 + 0x1 参数（兼容参数位置差异）
            re.compile(
                r'openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;.*?0x1|0x1.*?openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;',
                re.IGNORECASE | re.DOTALL
            ),
            # 4. 匹配常量引用（部分代码用常量名而非直接写 0x1，如 MODE_WORLD_READABLE）
            re.compile(
                r'Landroid/content/Context;->MODE_WORLD_READABLE\s*.+?openFileOutput',
                re.IGNORECASE | re.DOTALL
            )
        ],
        ...
        "mode_explanation": {
            "MODE_WORLD_READABLE": "0x1 (0001) - All apps can read the file",
            "MODE_PRIVATE": "0x0 (0000) - Only current app can access (safe)",
            "MODE_WORLD_WRITEABLE": "0x2 (0010) - All apps can write (high risk, often combined with readable)",
            "Combination Example": "0x3 = 0x1 | 0x2 (both read and write for all apps)"
        }
    }
}
...
def save_api_class_map(api_class_map: ApiClassMap) -> bool:
    ...
    try:
        json_content = {
            "description": "API-Class map for openFileOutput(..., MODE_WORLD_READABLE) vulnerability detection",
...
def analyze_vulnerability(api_class_map: ApiClassMap) -> Tuple[bool, Dict[str, List[Dict[str, Union[str, List[int]]]]]]:
    """Detect vulnerability by checking openFileOutput + MODE_WORLD_READABLE calls"""
...
def process_device(device: str) -> bool:
    ...
    if is_vulnerable:
        logging.error("[VULNERABLE] Context.openFileOutput(..., MODE_WORLD_READABLE) detected!")
        ...
        logging.error(f"\nRisk Explanation:")
        logging.error(f"1. MODE_WORLD_READABLE (0x1) makes the file readable by ALL apps on the device")
        logging.error(f"2. Critical risks:")
        logging.error(
            f"   - Attacker apps can read sensitive data in the file (e.g., user passwords, tokens, privacy information, configuration data)")
        logging.error(
            f"   - Compromises app sandbox isolation (private files should only be accessible to the app itself)")
        logging.error(f"   - If combined with MODE_WORLD_WRITEABLE (0x2), attackers can modify the file (worse impact)")
        ...
        logging.error(
            f"   - Android 7.0 (API 24+) enforces file sandbox by default, but legacy apps may still have risks")
        logging.error(
            f"   - Android 10+ (API 29+) further restricts file access, but vulnerable code still poses risks for lower versions")

        logging.error(f"\nFix Recommendation:")
        logging.error(f"1. Use safe file mode: Replace MODE_WORLD_READABLE with MODE_PRIVATE (0x0, default)")
        logging.error(f"   - Code example: openFileOutput(\"filename.txt\", Context.MODE_PRIVATE);")
        logging.error(f"   - MODE_PRIVATE: Only the current app can read/write the file (fully isolated)")
        logging.error(f"2. Avoid world-accessible modes: Never use MODE_WORLD_READABLE or MODE_WORLD_WRITEABLE")
        logging.error(f"3. For shared files (if necessary):")
        logging.error(f"   - Use ContentProvider with proper permission checks (grant access only to trusted apps)")
        logging.error(f"   - Use MediaStore for media files (images, videos, audio) with system-managed permissions")
        logging.error(
            f"   - Use external storage with scoped storage (Android 10+) instead of private files for sharing")
        logging.error(f"4. Secure sensitive data:")
        logging.error(f"   - Encrypt sensitive data before writing to files (even with MODE_PRIVATE)")
        logging.error(
            f"   - Avoid storing highly sensitive data (e.g., encryption keys) in files; use KeyStore instead")
        logging.error(f"5. Clean up legacy code: Remove deprecated MODE_WORLD_READABLE references in the codebase")
        return True
    else:
        logging.info("[SAFE] No openFileOutput(..., MODE_WORLD_READABLE) calls detected")
        logging.info(f"Target API: {list(TARGET_APIS.keys())[0]}")
        logging.info(f"Note: Ensure file mode uses MODE_PRIVATE (safe default) for private file storage")
        return False


def run_check():
    parser = argparse.ArgumentParser(
        description="Detect openFileOutput(..., MODE_WORLD_READABLE) File Exposure Vulnerability (auto-save API map)")
