...
# -*- coding: utf-8 -*-
"""
detect_openFileOutput_world_writeable_vuln.py

Detect openFileOutput(..., MODE_WORLD_WRITEABLE) Vulnerability in CarSetting.apk
Vulnerability Description:
Context.openFileOutput(String name, int mode) 用于创建/写入应用私有文件。
若第二个参数包含 MODE_WORLD_WRITEABLE（值为 0x02），表示文件对设备上所有应用可写，
攻击者可通过其他应用篡改该文件内容（如植入恶意代码、修改配置信息、伪造数据），导致应用逻辑异常、敏感数据泄露甚至远程代码执行。

Key Features:
1. Build JSON-format API-Class map (saved as APIMap_CarSetting_openFileOutput_writeable.json)
2. Detect openFileOutput calls with MODE_WORLD_WRITEABLE (core vulnerable configuration)
...
"""
POC_TAG = "30. 检测设备app是否使用了不安全的文件全局可写访问接口..."
...
JSON_OUTPUT_PATH = os.path.join(os.getcwd(), "APIMap_CarSetting_openFileOutput_writeable.json")

# ==========================
# Configuration: Target API (openFileOutput + MODE_WORLD_WRITEABLE)
# 核心API：Context.openFileOutput(String name, int mode)
# 方法签名：Landroid/content/Context;->openFileOutput(Ljava/lang/String;I)Ljava/io/FileOutputStream;
...
# ==========================
TARGET_APIS = {
    "Context.openFileOutput(..., MODE_WORLD_WRITEABLE)": {
        "smali_patterns": [
            # 1. 精确匹配 mode = MODE_WORLD_WRITEABLE（单独使用，值 0x2）
            re.compile(
                r'invoke-virtual\s*\{\s*.+?,\s*.+?,\s*0x2\s*\}\s*Landroid/content/Context;->openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;',
                re.IGNORECASE | re.DOTALL
            ),
            # 2. 匹配 mode 组合（如 0x2 | 0x1 = 0x3，只要包含 0x2 即匹配）
            re.compile(
                r'Landroid/content/Context;->openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;\s*.+?(0x[2367acef])',
                re.IGNORECASE | re.DOTALL
            ),
            # 3. 模糊匹配：方法签名 + 0x2 参数（兼容参数位置差异）
            re.compile(
                r'openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;.*?0x2|0x2.*?openFileOutput\(Ljava/lang/String;I\)Ljava/io/FileOutputStream;',
                re.IGNORECASE | re.DOTALL
            ),
            # 4. 匹配常量引用（部分代码用常量名而非直接写 0x2，如 MODE_WORLD_WRITEABLE）
            re.compile(
                r'Landroid/content/Context;->MODE_WORLD_WRITEABLE\s*.+?openFileOutput',
                re.IGNORECASE | re.DOTALL
            )
        ],
        "full_method": "Landroid/content/Context;->openFileOutput(Ljava/lang/String;I)Ljava/io/FileOutputStream;",
        "vulnerable_param": "mode contains MODE_WORLD_WRITEABLE (0x2, world-writable permission)",
        "mode_explanation": {
            "MODE_WORLD_WRITEABLE": "0x2 (0010) - All apps can write to the file",
            "MODE_PRIVATE": "0x0 (0000) - Only current app can access (safe)",
            "MODE_WORLD_READABLE": "0x1 (0001) - All apps can read the file (high risk)",
            "Combination Example": "0x3 = 0x1 | 0x2 (both read and write for all apps - critical risk)"
        }
    }
}
...
def save_api_class_map(api_class_map: ApiClassMap) -> bool:
    ...
    try:
        json_content = {
            "description": "API-Class map for openFileOutput(..., MODE_WORLD_WRITEABLE) vulnerability detection",
...
def analyze_vulnerability(api_class_map: ApiClassMap) -> Tuple[bool, Dict[str, List[Dict[str, Union[str, List[int]]]]]]:
    """Detect vulnerability by checking openFileOutput + MODE_WORLD_WRITEABLE calls"""
...
def process_device(device: str) -> bool:
    ...
    if is_vulnerable:
        logging.error("[CRITICAL VULNERABILITY] Context.openFileOutput(..., MODE_WORLD_WRITEABLE) detected!")
        ...
        logging.error(f"\nRisk Explanation:")
        logging.error(f"1. MODE_WORLD_WRITEABLE (0x2) makes the file writable by ALL apps on the device")
        logging.error(f"2. Critical risks (higher than read-only):")
        logging.error(
            f"   - Attacker apps can篡改 file content (e.g., modify configuration files to bypass authentication, inject malicious code)")
        logging.error(
            f"   - Forge sensitive data (e.g., fake user credentials, tampered logs) leading to application logic errors")
        logging.error(
            f"   - If combined with MODE_WORLD_READABLE (0x3), attackers can read + write (full control over the file)")
        logging.error(f"   - Potential remote code execution: If the app loads/execute code from the tampered file")
        logging.error(f"3. Android version note:")
        logging.error(f"   - Android 4.2 (API 17+) deprecated MODE_WORLD_WRITEABLE/MODE_WORLD_READABLE")
        logging.error(f"   - Android 7.0 (API 24+) enforces file sandbox, but legacy apps may still be affected")
        logging.error(
            f"   - Android 10+ (API 29+) scoped storage further restricts access, but vulnerable code remains high risk for all versions")

        logging.error(f"\nFix Recommendation:")
        logging.error(
            f"1. Urgently replace with safe mode: Use MODE_PRIVATE (0x0, default) instead of MODE_WORLD_WRITEABLE")
        logging.error(f"   - Code example: openFileOutput(\"filename.txt\", Context.MODE_PRIVATE);")
        logging.error(
            f"   - MODE_PRIVATE: Only the current app can read/write the file (fully isolated from other apps)")
        logging.error(
            f"2. Strictly prohibit world-writable modes: Never use MODE_WORLD_WRITEABLE (deprecated and extremely dangerous)")
        logging.error(f"3. For legitimate file sharing (if required):")
        logging.error(
            f"   - Use ContentProvider with fine-grained permission checks (e.g., grant write access only to trusted apps via custom permissions)")
        logging.error(f"   - Use MediaStore for media files (with system-managed write permissions)")
        logging.error(
            f"   - Use scoped storage (Android 10+) for external file sharing (avoids exposing private files)")
        logging.error(f"4. Enhance file security:")
        logging.error(f"   - Encrypt all sensitive files (even with MODE_PRIVATE) to prevent tampering/reading")
        logging.error(
            f"   - Add file integrity checks (e.g., MD5/SHA-256 hash) to detect tampered content before using it")
        logging.error(
            f"   - Avoid storing executable code, configuration files, or critical data in world-accessible paths")
        logging.error(
            f"5. Clean up legacy code: Remove all references to MODE_WORLD_WRITEABLE and audit related file operations")
        logging.error(
            f"6. Conduct security testing: After fixing, verify that no other apps can access/write the app's private files")
        return True
    else:
        logging.info("[SAFE] No openFileOutput(..., MODE_WORLD_WRITEABLE) calls detected")
        logging.info(f"Target API: {list(TARGET_APIS.keys())[0]}")
        logging.info(f"Note: Ensure all file operations use MODE_PRIVATE and avoid deprecated world-accessible modes")
        return False


def run_check():
    parser = argparse.ArgumentParser(
        description="Detect openFileOutput(..., MODE_WORLD_WRITEABLE) File Tampering Vulnerability (auto-save API map)")
...
class Poc30OpenfileAnywPlugin(IVIVulnerabilityPlugin):
    meta_poc_name = '检测设备app是否使用了不安全的文件全局可写访问接口...'
    meta_cve_id = 'CWE-732'
    meta_severity = 'High'
