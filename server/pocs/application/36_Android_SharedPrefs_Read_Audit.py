...
# -*- coding: utf-8 -*-
"""
detect_getSharedPreferences_world_readable_vuln.py

Detect getSharedPreferences(..., MODE_WORLD_READABLE) Vulnerability in CarSetting.apk
Vulnerability Description:
Context.getSharedPreferences(String name, int mode) 用于获取应用的偏好设置存储实例。
若第二个参数包含 MODE_WORLD_READABLE（值为 0x01），表示偏好设置文件（.xml 格式）对设备上所有应用可读，
攻击者可通过其他应用读取其中存储的敏感配置（如用户账号、密码、Token、隐私设置等），导致信息泄露。

Key Features:
1. Build JSON-format API-Class map (saved as APIMap_CarSetting_getSharedPreferences.json)
2. Detect getSharedPreferences calls with MODE_WORLD_READABLE (core vulnerable configuration)
...
"""
POC_TAG = "31. 检测设备app是否使用了不安全的数据库全局可读访问接口..."
...
JSON_OUTPUT_PATH = os.path.join(os.getcwd(), "APIMap_CarSetting_getSharedPreferences.json")

# ==========================
# Configuration: Target API (getSharedPreferences + MODE_WORLD_READABLE)
...
# 漏洞点：mode 参数包含 MODE_WORLD_READABLE（值 0x01，二进制 0001）
# 注意：mode 支持组合（如 MODE_WORLD_READABLE | MODE_MULTI_PROCESS = 0x05），只要包含 0x01 即漏洞
# ==========================
TARGET_APIS = {
    "Context.getSharedPreferences(..., MODE_WORLD_READABLE)": {
        "smali_patterns": [
            # 1. 精确匹配 mode = MODE_WORLD_READABLE（单独使用，值 0x1）
            re.compile(
                r'invoke-virtual\s*\{\s*.+?,\s*.+?,\s*0x1\s*\}\s*Landroid/content/Context;->getSharedPreferences\(Ljava/lang/String;I\)Landroid/content/SharedPreferences;',
                re.IGNORECASE | re.DOTALL
            ),
            # 2. 匹配 mode 组合（如 0x1 | 0x4 = 0x5，只要包含 0x1 即匹配）
            re.compile(
                r'Landroid/content/Context;->getSharedPreferences\(Ljava/lang/String;I\)Landroid/content/SharedPreferences;\s*.+?(0x[13579bdf])',
                re.IGNORECASE | re.DOTALL
            ),
            # 3. 模糊匹配：方法签名 + 0x1 参数（兼容参数位置差异）
            re.compile(
                r'getSharedPreferences\(Ljava/lang/String;I\)Landroid/content/SharedPreferences;.*?0x1|0x1.*?getSharedPreferences\(Ljava/lang/String;I\)Landroid/content/SharedPreferences;',
                re.IGNORECASE | re.DOTALL
            ),
            # 4. 匹配常量引用（部分代码用常量名而非直接写 0x1，如 MODE_WORLD_READABLE）
            re.compile(
                r'Landroid/content/Context;->MODE_WORLD_READABLE\s*.+?getSharedPreferences',
                re.IGNORECASE | re.DOTALL
            )
        ],
        "full_method": "Landroid/content/Context;->getSharedPreferences(Ljava/lang/String;I)Landroid/content/SharedPreferences;",
        "vulnerable_param": "mode contains MODE_WORLD_READABLE (0x1, world-readable permission)",
        "mode_explanation": {
            "MODE_WORLD_READABLE": "0x1 (0001) - All apps can read the SharedPreferences file",
            "MODE_PRIVATE": "0x0 (0000) - Only current app can access (safe, default)",
            "MODE_WORLD_WRITEABLE": "0x2 (0010) - All apps can write (critical risk)",
            "MODE_MULTI_PROCESS": "0x4 (0100) - Deprecated, for multi-process access (combined with 0x1 still vulnerable)",
            "Combination Example": "0x5 = 0x1 | 0x4 (world-readable + multi-process)"
        }
    }
}
...
def save_api_class_map(api_class_map: ApiClassMap) -> bool:
    ...
    try:
        json_content = {
            "description": "API-Class map for getSharedPreferences(..., MODE_WORLD_READABLE) vulnerability detection",
...
def analyze_vulnerability(api_class_map: ApiClassMap) -> Tuple[bool, Dict[str, List[Dict[str, Union[str, List[int]]]]]]:
    """Detect vulnerability by checking getSharedPreferences + MODE_WORLD_READABLE calls"""
...
def process_device(device: str) -> bool:
    ...
    if is_vulnerable:
        logging.error("[VULNERABLE] Context.getSharedPreferences(..., MODE_WORLD_READABLE) detected!")
        ...
        logging.error(f"\nRisk Explanation:")
        logging.error(
            f"1. SharedPreferences is commonly used to store sensitive configuration data (e.g., user credentials, API tokens, privacy settings, app states)")
        logging.error(
            f"2. MODE_WORLD_READABLE (0x1) allows ALL apps on the device to read the SharedPreferences file (stored as XML in app's private directory)")
        logging.error(f"3. Key risks:")
        logging.error(
            f"   - Leakage of user credentials (usernames, passwords, session tokens) leading to account compromise")
        logging.error(
            f"   - Exposure of sensitive configuration (e.g., API keys, encryption secrets) enabling further attacks")
        logging.error(f"   - Privacy violation (leakage of user behavior data, preferences, or personal information)")
        logging.error(
            f"   - If combined with MODE_WORLD_WRITEABLE (0x3), attackers can modify preferences (critical risk)")
        logging.error(f"4. Android version note:")
        logging.error(f"   - Android 4.2 (API 17+) deprecated MODE_WORLD_READABLE/MODE_WORLD_WRITEABLE")
        logging.error(
            f"   - Android 7.0 (API 24+) enforces app sandbox, but legacy apps may still leak data to privileged apps")
        logging.error(
            f"   - Android 10+ (API 29+) further restricts file access, but SharedPreferences with world-readable mode remains risky")

        logging.error(f"\nFix Recommendation:")
        logging.error(f"1. Use safe mode: Replace MODE_WORLD_READABLE with MODE_PRIVATE (0x0, default)")
        logging.error(f"   - Code example: getSharedPreferences(\"my_prefs\", Context.MODE_PRIVATE);")
        logging.error(
            f"   - MODE_PRIVATE: Only the current app can read/write the SharedPreferences file (fully isolated)")
        logging.error(
            f"2. Never use world-accessible modes: Avoid MODE_WORLD_READABLE and MODE_WORLD_WRITEABLE (both deprecated and insecure)")
        logging.error(f"3. Secure sensitive data in SharedPreferences:")
        logging.error(
            f"   - Encrypt sensitive values (e.g., passwords, tokens) before storing (use AndroidX Security库's EncryptedSharedPreferences)")
        logging.error(
            f"   - Example: EncryptedSharedPreferences provides transparent encryption/decryption of preferences")
        logging.error(
            f"   - Avoid storing highly sensitive data (e.g., encryption keys) in SharedPreferences; use KeyStore instead")
        logging.error(f"4. For shared preferences (if required):")
        logging.error(f"   - Use ContentProvider with strict permission checks to share data only with trusted apps")
        logging.error(f"   - Use custom permissions to restrict access to the shared data")
        logging.error(
            f"5. Clean up legacy code: Remove all references to MODE_WORLD_READABLE and audit SharedPreferences usage")
        logging.error(
            f"6. Audit existing data: Check if sensitive data was already leaked via world-readable preferences and take remediation (e.g., revoke tokens)")
        return True
    else:
        logging.info("[SAFE] No getSharedPreferences(..., MODE_WORLD_READABLE) calls detected")
        logging.info(f"Target API: {list(TARGET_APIS.keys())[0]}")
        logging.info(f"Note: Ensure SharedPreferences uses MODE_PRIVATE and encrypt sensitive data if stored")
        return False


def run_check():
    parser = argparse.ArgumentParser(
        description="Detect getSharedPreferences(..., MODE_WORLD_READABLE) Data Leak Vulnerability (auto-save API map)")
    ...
    sys.exit(1 if overall_vulnerable else 0)
...
class Poc31SharedprefsAnyrPlugin(IVIVulnerabilityPlugin):
    meta_poc_name = '检测设备app是否使用了不安全的数据库全局可读访问接口...'
    meta_cve_id = 'CWE-732'
    meta_severity = 'High'
