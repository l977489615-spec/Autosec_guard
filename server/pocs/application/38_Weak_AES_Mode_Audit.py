...
# -*- coding: utf-8 -*-
"""
detect_weak_aes_vuln.py

Detect Weak AES Modes (AES/ECB/NoPadding, AES/ECB/PKCS5Padding) in CarSetting.apk
Vulnerability Description:
AES encryption using ECB (Electronic Codebook) mode is insecure because it lacks randomness—identical plaintext blocks produce identical ciphertext blocks.
This allows attackers to:
1. Deduce patterns in sensitive data (e.g., vehicle VIN, user credentials)
2. Perform differential cryptanalysis to recover plaintext
3. Tamper with encrypted data without detection
Key unsafe modes:
- AES/ECB/NoPadding: ECB mode with no padding (vulnerable to block alignment attacks)
- AES/ECB/PKCS5Padding: ECB mode with PKCS5 padding (still lacks randomness)

Key Features:
1. Build JSON-format API-Class map (saved as APIMap_CarSetting_WeakAES.json)
2. Detect Cipher.getInstance() calls with unsafe AES-ECB modes
...
6. Support ADB pull, APK decompilation (apktool), and batch device processing
"""
POC_TAG = "33. 检测设备app是否使用了不安全的加密算法配置（ECB、NO/PKCS5 Padding）..."
...
JSON_OUTPUT_PATH = os.path.join(os.getcwd(), "APIMap_CarSetting_WeakAES.json")

# ==========================
# Configuration: Weak AES Modes (ECB)
# 核心API：javax.crypto.Cipher.getInstance(String transformation)
# 不安全模式：AES/ECB/NoPadding、AES/ECB/PKCS5Padding（大小写不敏感，支持动态拼接）
# 方法签名：Ljava/security/Key;->getInstance(Ljava/lang/String;)Ljava/security/Key; (Key生成)
#          Ljavax/crypto/Cipher;->getInstance(Ljava/lang/String;)Ljavax/crypto/Cipher; (Cipher初始化)
# ==========================
TARGET_APIS = {
    "AES/ECB/NoPadding (Weak Mode)": {
        "smali_patterns": [
            # 1. 精确匹配 Cipher.getInstance("AES/ECB/NoPadding")（大小写不敏感）
            re.compile(
                r'invoke-static\s*\{\s*.+?(aes/ecb/nopadding).+?\}\s*Ljavax/crypto/Cipher;->getInstance\(Ljava/lang/String;\)Ljavax/crypto/Cipher;',
                re.IGNORECASE | re.DOTALL
            ),
            # 2. 匹配动态拼接（如 "AES/" + "ECB" + "/NoPadding"）
            re.compile(
                r'(aes).+?(ecb).+?(nopadding).+?Ljavax/crypto/Cipher;->getInstance',
                re.IGNORECASE | re.DOTALL
            ),
            # 3. 匹配常量引用（如 CONST_STRING 定义模式字符串）
            re.compile(
                r'const-string\s+.+?,\s*".*?(AES/ECB/NoPadding).*?"\s*.+?Cipher\.getInstance',
                re.IGNORECASE | re.DOTALL
            )
        ],
        "full_method": "Ljavax/crypto/Cipher;->getInstance(Ljava/lang/String;)Ljavax/crypto/Cipher;",
        "vulnerable_desc": "AES encryption using ECB mode with no padding (insecure, lacks randomness)",
        "risk_level": "CRITICAL"
    },
    "AES/ECB/PKCS5Padding (Weak Mode)": {
        "smali_patterns": [
            # 1. 精确匹配 Cipher.getInstance("AES/ECB/PKCS5Padding")（大小写不敏感）
            re.compile(
                r'invoke-static\s*\{\s*.+?(aes/ecb/pkcs5padding).+?\}\s*Ljavax/crypto/Cipher;->getInstance\(Ljava/lang/String;\)Ljavax/crypto/Cipher;',
                re.IGNORECASE | re.DOTALL
            ),
            # 2. 匹配动态拼接（如 "AES" + "/" + "ECB" + "/PKCS5Padding"）
            re.compile(
                r'(aes).+?(ecb).+?(pkcs5padding).+?Ljavax/crypto/Cipher;->getInstance',
                re.IGNORECASE | re.DOTALL
            ),
            # 3. 匹配常量引用
            re.compile(
                r'const-string\s+.+?,\s*".*?(AES/ECB/PKCS5Padding).*?"\s*.+?Cipher\.getInstance',
                re.IGNORECASE | re.DOTALL
            )
        ],
        "full_method": "Ljavax/crypto/Cipher;->getInstance(Ljava/lang/String;)Ljavax/crypto/Cipher;",
        "vulnerable_desc": "AES encryption using ECB mode with PKCS5 padding (insecure, identical plaintext → identical ciphertext)",
        "risk_level": "CRITICAL"
    }
}
...
def save_api_class_map(api_class_map: ApiClassMap) -> bool:
    ...
    try:
        json_content = {
            "description": "API-Class map for Weak AES (ECB Mode) Vulnerability Detection",
...
def analyze_vulnerability(api_class_map: ApiClassMap) -> Tuple[bool, Dict[str, List[Dict[str, Union[str, List[int]]]]]]:
    """Detect vulnerability by checking weak AES-ECB mode calls"""
...
def process_device(device: str) -> bool:
    ...
    if is_vulnerable:
        logging.error("[CRITICAL VULNERABILITY] Weak AES-ECB Modes Detected!")
        for api_name, callers in vulnerable_api_details.items():
            api_config = TARGET_APIS[api_name]
            logging.error(f"\nVulnerable Mode: {api_name}")
            logging.error(f"Risk Level: {api_config['risk_level']}")
            logging.error(f"Description: {api_config['vulnerable_desc']}")
        ...
        logging.error(f"\nRisk Explanation (CarSetting.apk Specific):")
        logging.error(f"1. ECB Mode Fundamental Flaw:")
        logging.error(f"   - Identical plaintext blocks → identical ciphertext blocks (no randomness)")
        logging.error(f"   - Attackers can recover sensitive data (e.g., vehicle VIN, user credentials, diagnostic logs) via pattern analysis")
        logging.error(f"2. Car-Specific Risks:")
        logging.error(f"   - Tamper with encrypted vehicle settings (e.g., door lock status, cruise control parameters)")
        logging.error(f"   - Leak sensitive diagnostic data (ECU logs, fault codes) leading to vehicle tampering")
        logging.error(f"   - Compromise user privacy (navigation history, driving behavior data)")
        logging.error(f"3. No Padding vs PKCS5 Padding:")
        logging.error(f"   - AES/ECB/NoPadding: Vulnerable to block alignment attacks (plaintext length must be multiple of 16 bytes)")
        logging.error(f"   - AES/ECB/PKCS5Padding: Padding does not fix randomness issue (still insecure for sensitive data)")

        logging.error(f"\nFix Recommendation (Car-Specific Security Requirements):")
        logging.error(f"1. Replace ECB Mode with Secure Alternatives (Mandatory):")
        logging.error(f"   - Recommended Mode: AES/GCM/NoPadding (AEAD mode, provides confidentiality + integrity + authenticity)")
        logging.error(f"   - Alternative Mode: AES/CBC/PKCS5Padding (requires random 16-byte IV + IV storage with ciphertext)")
        logging.error(f"   - Code Example (GCM Mode):")
        logging.error(f"     Cipher cipher = Cipher.getInstance(\"AES/GCM/NoPadding\");")
        logging.error(f"     SecretKeySpec keySpec = new SecretKeySpec(aesKey, \"AES\");")
        logging.error(f"     GCMParameterSpec gcmSpec = new GCMParameterSpec(128, generateRandomIV(12)); // 12-byte IV (recommended)")
        logging.error(f"     cipher.init(Cipher.ENCRYPT_MODE, keySpec, gcmSpec);")
        logging.error(f"2. Key Management (Critical for Vehicle Security):")
        logging.error(f"   - Store AES keys in Android KeyStore (not in SharedPreferences or plaintext files)")
        logging.error(f"   - Use hardware-backed KeyStore (if available) to prevent key extraction")
        logging.error(f"   - Generate unique keys per vehicle/user (avoid hardcoded keys)")
        logging.error(f"3. IV/Nonce Requirements:")
        logging.error(f"   - Never reuse IV/Nonce for AES-GCM/CBC (generates predictable ciphertext)")
        logging.error(f"   - For GCM: Use 12-byte IV (optimal performance + security) and 128-bit authentication tag")
        logging.error(f"   - Embed IV with ciphertext (IV is not secret, but must be unique)")
        logging.error(f"4. CarSetting.apk Specific Fixes:")
        logging.error(f"   - Audit all Cipher.getInstance() calls in smali files (focus on encryption of vehicle/data/user data)")
        logging.error(f"   - Replace all ECB mode calls with GCM/CBC mode")
        logging.error(f"   - Add integrity verification for encrypted vehicle data (GCM mode provides this natively)")
        logging.error(f"5. Post-Fix Validation:")
        logging.error(f"   - Verify no ECB mode calls remain (re-run this script to confirm)")
        logging.error(f"   - Test encryption with different plaintexts (ensure ciphertext is random even for similar inputs)")
        logging.error(f"   - Conduct penetration testing to validate against pattern analysis attacks")
        return True
    else:
        logging.info("[SAFE] No Weak AES-ECB Modes Detected")
        logging.info(f"Target Weak Modes: {list(TARGET_APIS.keys())}")
        logging.info(f"Note: Ensure AES keys are stored in KeyStore and IV/Nonce is generated randomly for secure modes (GCM/CBC)")
        return False



def run_check():
    # 导入datetime（修复JSON报告中的时间戳）
    global datetime
    from datetime import datetime

    parser = argparse.ArgumentParser(
        description="Detect Weak AES-ECB Modes (AES/ECB/NoPadding, AES/ECB/PKCS5Padding) in CarSetting.apk")
