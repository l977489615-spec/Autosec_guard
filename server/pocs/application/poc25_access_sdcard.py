...
# -*- coding: utf-8 -*-
"""
detect_sdcard_access_vuln.py

Detect SD card data access vulnerability in CarSetting.apk (via getExternalStorageDirectory call)
Vulnerability Description: Apps calling Environment.getExternalStorageDirectory() write/read data to public SD card,
which can be accessed by third-party apps without restrictions. This leads to sensitive data leakage or unauthorized modification.

Key Features:
1. Build JSON-format API-Class map (saved as APIMap_CarSetting.json in current directory)
2. Vulnerability detection directly queries the map (no repeated smali traversal)
3. Reuse existing decoded folders with prefix "decoded_CarSetting.apk_"
4. Compatible with Python 3.6+
5. Auto-save API-Class map to APIMap_CarSetting.json (no extra parameters needed)
"""
POC_TAG = "25. 检测设备app是否使用了不安全的外部数据存储..."
...
JSON_OUTPUT_PATH = os.path.join(os.getcwd(), "APIMap_CarSetting_poc25.json")

# ==========================
# Configuration: Target APIs
# ==========================
TARGET_APIS = {
    "Environment.getExternalStorageDirectory()": {
        "smali_patterns": [
            re.compile(
                r'invoke-static\s*\{\}\s*Landroid/os/Environment;->getExternalStorageDirectory\(\)Ljava/io/File;',
                re.IGNORECASE),
            re.compile(r'getExternalStorageDirectory\(\)Ljava/io/File;', re.IGNORECASE),
            re.compile(r'Landroid/os/Environment;->getExternalStorageDirectory', re.IGNORECASE)
        ],
        "full_method": "Landroid/os/Environment;->getExternalStorageDirectory()Ljava/io/File;"
    }
    # Add new APIs here (example):
    # "Environment.getExternalStoragePublicDirectory(String)": {
    #     "smali_patterns": [
    #         re.compile(r'invoke-static\s*\{.+\}\s*Landroid/os/Environment;->getExternalStoragePublicDirectory\(Ljava/lang/String;\)Ljava/io/File;', re.IGNORECASE)
    #     ],
    #     "full_method": "Landroid/os/Environment;->getExternalStoragePublicDirectory(Ljava/lang/String;)Ljava/io/File;"
    # }
}
...
def find_existing_decoded_folder() -> Optional[str]:
    ...
    for item in os.listdir(cwd):
        ...
        if os.path.isdir(item_path) and item.startswith(DECODED_FOLDER_PREFIX):
            smali_dir = os.path.join(item_path, "smali")
            if os.path.exists(smali_dir) and os.listdir(smali_dir):
                logging.info(f"Found valid existing decoded folder: {item_path}")
                return item_path
...
def try_apktool_decode(apk_path: str, out_dir: str, apktool_path: str) -> bool:
    ...
    if not os.path.exists(smali_dir) or not os.listdir(smali_dir):
        logging.error("Decompilation succeeded but smali directory is missing/empty")
        return False
    logging.info(f"APK decompiled successfully: {out_dir}")
    return True


def build_api_class_map(smali_root: str) -> ApiClassMap:
    """Build API-Class map by scanning smali code once"""
    api_class_map: ApiClassMap = {api_name: [] for api_name in TARGET_APIS.keys()}

    logging.info(f"\nBuilding API-Class map (scanning smali files in: {smali_root})")
...
def save_api_class_map(api_class_map: ApiClassMap) -> bool:
    """Auto-save API-Class map to APIMap_CarSetting.json (fixed path)"""
    try:
        with open(JSON_OUTPUT_PATH, "w", encoding="utf-8") as f:
            json.dump(api_class_map, f, ensure_ascii=False, indent=2)
...
def get_or_create_decoded_folder(apk_path: str, device: str) -> Optional[str]:
    ...
    logging.info(f"\nDecompiling to new folder: {new_decoded_dir}")
...
def analyze_vulnerability(api_class_map: ApiClassMap) -> Tuple[bool, Dict[str, List[Dict[str, Union[str, List[int]]]]]]:
    """Detect vulnerability by querying API-Class map"""
...
def process_device(device: str) -> bool:
    ...
    local_apk = local_apk_exists()
    apk_path = local_apk
    if not local_apk:
        logging.info(f"No local {TARGET_APK_NAME} found. Pulling from device...")
        for remote_path in REMOTE_APK_PATHS:
            if remote_path_exists(device, remote_path):
                ...
                if adb_pull(device, remote_path, local_apk_path):
                    ...
                    logging.info(f"Pulled APK: {local_apk_path}")
                    break
                else:
                    logging.warning(f"Failed to pull from {remote_path}")
        if not apk_path:
    ...
    # Step 3: Build API-Class map + auto-save to JSON
    smali_root = os.path.join(decoded_dir, "smali")
    api_class_map = build_api_class_map(smali_root)
    save_api_class_map(api_class_map)  # Auto-save, no extra parameter

    # Step 4: Detect vulnerability via map query
    logging.info(f"\n===== Vulnerability Detection Result =====")
    is_vulnerable, vulnerable_api_details = analyze_vulnerability(api_class_map)

    if is_vulnerable:
        logging.error("[VULNERABLE] Target vulnerable API(s) detected:")
        for api_name, callers in vulnerable_api_details.items():
            logging.error(f"\nAPI: {api_name}")
            logging.error(f"Called by {len(callers)} class(es):")
            for caller in callers:
                logging.error(f"  - File: {caller['smali_file']}")
                logging.error(f"    Lines: {', '.join(map(str, caller['line_numbers']))}")
        logging.error(f"\nRisk Explanation:")
        logging.error(
            f"- {list(TARGET_APIS.keys())[0]} accesses public SD card, which is accessible to third-party apps")
        logging.error(f"- Sensitive data stored here may be leaked or modified by unauthorized apps")
        logging.error(f"\nFix Recommendation:")
        logging.error(f"1. Replace with app-specific private storage: context.getExternalFilesDir(null)")
        logging.error(f"2. Use MediaStore API for shared media files")
        logging.error(f"3. Avoid storing sensitive data (passwords, tokens) in external storage")
        return True
    else:
        logging.info("[SAFE] No vulnerable API calls detected")
        logging.info(f"- None of the target APIs ({', '.join(TARGET_APIS.keys())}) are used")
        return False


def main():
    parser = argparse.ArgumentParser(
        description="Detect SD Card Access Vulnerability (auto-save API map to APIMap_CarSetting.json)")
