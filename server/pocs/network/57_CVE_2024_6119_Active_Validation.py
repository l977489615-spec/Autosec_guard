...
def _probe_cert_san(host: str, port: int) -> dict:
    """Inspect TLS certificate SAN for otherName entries (X.509 name check trigger)."""
    result = {}
    try:
        import ssl
        import socket
        ...
        with socket.create_connection((host, port), timeout=6) as raw_sock:
            with ctx.wrap_socket(raw_sock, server_hostname=host) as tls_sock:
                ...
                if cert:
                    ...
                    result["cert_san_types"] = [t for t, _ in sans]
                    result["has_otherName_san"] = any(t == "otherName" for t, _ in sans)
                    result["cert_san_count"] = len(sans)
                result["tls_version"] = tls_sock.version()
                result["cipher"] = tls_sock.cipher()
    except Exception as exc:
        result["cert_probe_error"] = str(exc)
    return result
...
def _run_poc(plugin) -> dict:
    ...
    evidence = {
        ...
        "technique": "TLS fingerprinting + X.509 SAN inspection + semantic version comparison",
    ...
    # Step 2: X.509 SAN inspection (look for otherName entries - CVE trigger)
    cert_info = _probe_cert_san(target_ip, port)
    evidence.update({k: v for k, v in cert_info.items() if v is not None})
...
class Poc57CVE20246119DoSAuditPlugin(IVIVulnerabilityPlugin):
    meta_display_id = "POC-NET-057"
    meta_poc_name = 'CVE-2024-6119 验证问题 Active Validation'
    ...
    meta_references       = ['https://nvd.nist.gov/vuln/detail/CVE-2024-6119']
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "57_OpenSSL_X509_Name_Check_DoS_Audit") if "VULN" in dir() else "57_OpenSSL_X509_Name_Check_DoS_Audit"
    ...
    _plugin = Poc57CVE20246119DoSAuditPlugin({
