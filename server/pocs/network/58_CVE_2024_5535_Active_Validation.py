    ...
    "affected": [
        {
            ...
            "versions": [
                ...
                {"version": "1.0.1", "status": "affected", "lessThan": "3.0.0", "versionType": "semver"},
            ]
        }
    ],
    "signature_tokens": [
...
def _probe_alpn_npn(host: str, port: int) -> dict:
    """Probe TLS ALPN/NPN support - trigger NextProtocol negotiation path."""
    result = {}
    try:
        import ssl
        import socket
        ...
        ctx.set_alpn_protocols(["h2", "http/1.1", "spdy/3.1"])
        with socket.create_connection((host, port), timeout=6) as raw_sock:
            with ctx.wrap_socket(raw_sock, server_hostname=host) as tls_sock:
                ...
                result["alpn_probe_success"] = True
    except Exception as exc:
        result["alpn_probe_error"] = str(exc)
    return result
...
def _run_poc(plugin) -> dict:
    ...
    evidence = {
        ...
        "technique": "TLS fingerprinting + ALPN/NPN negotiation probe + semantic version comparison",
        "reference": "https://www.openssl.org/news/secadv/20240627.txt",
    }
    ...
    tls_info = tls_get_server_info(target_ip, port, alpn=["h2", "http/1.1"])
    evidence.update({k: v for k, v in tls_info.items() if v})

    # Step 2: ALPN/NPN probe (exercise SSL_select_next_proto path)
    alpn_result = _probe_alpn_npn(target_ip, port)
    evidence.update({k: v for k, v in alpn_result.items() if v is not None})
...
class Poc58CVE20245535OutOfBoundsAuditPlugin(IVIVulnerabilityPlugin):
    ...
    meta_references       = ['https://nvd.nist.gov/vuln/detail/CVE-2024-5535']
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "58_OpenSSL_Next_Proto_Buffer_OOB_Audit") if "VULN" in dir() else "58_OpenSSL_Next_Proto_Buffer_OOB_Audit"
    ...
    _plugin = Poc58CVE20245535OutOfBoundsAuditPlugin({
