...
VULN = {
    ...
    "component":      "Unknown",
    "type":           "Unknown",
    "summary":        "Broadcom WME Overflow",
    "source_url":     "https://nvd.nist.gov/vuln/detail/CVE-2017-9417",
    "affected":       [{"vendor": "Unknown", "product": "Unknown", "versions": []}],
    "references":     ["https://nvd.nist.gov/vuln/detail/CVE-2017-9417"],
    "signature_tokens": ["CVE-2017-9417"],
}


def _run_poc(plugin, vuln=None) -> dict:
    """
    CVE-2017-9417 主动探针包装（兼容旧式 exploit() 实现）。
    ...
    evidence = {
        "cve":       vuln.get("cve", "CVE-2017-9417") if vuln else "CVE-2017-9417",
...
class BroadcomWMEPlugin(IVIVulnerabilityPlugin):
    ...
    meta_poc_name = 'CVE-2017-9417 Overflow Active Validation'
    ...
    def check_prerequisites(self):
        ...
        if self.params.get("allow_disruptive") not in (True, "true", "True", "1", 1):
            raise RuntimeError("Broadcom WME Overflow 仅允许在隔离实验环境中执行，需显式设置 allow_disruptive=true")
        return True
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 07_CVE_2017_9417_Overflow_Active_Validation.py <interface>")
        sys.exit(1)
