#!/usr/bin/env python3
"""CVE-2020-10063 – SweynTooth BLE: Invalid Channel Map – BLE SoC crash via invalid channel map field.

Public PoC source: https://github.com/Matheus-Garbelini/sweyntooth_bluetooth_low_energy_attacks
  Script: invalid_channel_map.py
...
Usage:
  python invalid_channel_map.py /dev/ttyACM0 <target_bdaddr>
"""
...
from probe_utils import detection_confidence
...
VULN = {
    ...
    "summary": "Invalid Channel Map – BLE SoC crash via invalid channel map field",
...
SWEYN_SCRIPT = SWEYN_REPO / "invalid_channel_map.py"
...
def _run_poc(plugin):
    ...
    evidence = {
        ...
        "attack_surface": "BLE SoC invalid channel map in CONNECT_IND PDU → crash",
    }
    ...
    return {
        ...
        "evidence": evidence,
        "requires_manual_review": True,
        "detection_confidence": detection_confidence("HW", evidence, "bluetooth_hardware_required"),
        "poc_source": f"Matheus-Garbelini/sweyntooth / {SWEYN_SCRIPT.name}",
    }


class Poc100CVE202010063SweynToothBleAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50_more"
    meta_display_id = "POC-WIRELESS-100"
    meta_poc_name   = 'CVE-2020-10063 SweynTooth Invalid Channel Map Active Validation'
    meta_cve_id     = "CVE-2020-10063"
    meta_severity   = "High"
    ...
    meta_attack_surface = "BLE SoC invalid channel map in CONNECT_IND PDU → crash"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "100_Invalid_Channel_Map__BLE_SoC_crash_via_Audit") if "VULN" in dir() else "100_Invalid_Channel_Map__BLE_SoC_crash_via_Audit"
    ...
    _plugin = Poc100CVE202010063SweynToothBleAuditPlugin({
