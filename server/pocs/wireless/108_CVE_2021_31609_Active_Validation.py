#!/usr/bin/env python3
"""CVE-2021-31609 – BrakTooth BT Classic: LMP Auto Rate Overflow – Silabs WT32i overflow in LMP auto rate.
...
VULN = {
    ...
    "year": 2021,
    "domain": "wireless",
    "vendor_product": "Silabs WT32i Bluetooth Classic SoC",
    "component": "Bluetooth Classic LMP / HCI stack",
    "type": "BT Classic protocol attack → DoS / crash / RCE",
    "summary": "LMP Auto Rate Overflow – Silabs WT32i overflow in LMP auto rate",
...
def _run_poc(plugin):
    ...
    evidence = {
        ...
        "attack_surface": "Silabs WT32i BT SoC LMP auto rate overflow → crash/reboot",
    }
...
class Poc108CVE202131609BrakToothBtClassicAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50_more"
    meta_display_id = "POC-WIRELESS-108"
    meta_poc_name   = 'CVE-2021-31609 BrakTooth WT32i LMP Auto Rate Overflow Active Validation'
    meta_cve_id     = "CVE-2021-31609"
    meta_severity   = "High"
    ...
    meta_attack_surface = "Silabs WT32i BT SoC LMP auto rate overflow → crash/reboot"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "108_LMP_Auto_Rate_Overflow__Silabs_WT32i_ov_Audit") if "VULN" in dir() else "108_LMP_Auto_Rate_Overflow__Silabs_WT32i_ov_Audit"
    ...
    _plugin = Poc108CVE202131609BrakToothBtClassicAuditPlugin({
