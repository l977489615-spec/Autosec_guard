...
Usage: python3 22_CVE_2019_9506_BT_KNOB_Key_Length_Active_Validation.py <target_mac> [bluetooth_version] [min_key_size]
"""
...
VULN = {
    ...
    "component":      "Unknown",
    "type":           "Unknown",
    "summary":        "BT KNOB Key Length Audit",
    "source_url":     "https://nvd.nist.gov/vuln/detail/CVE-2019-9506",
    "affected":       [{"vendor": "Unknown", "product": "Unknown", "versions": []}],
    "references":     ["https://nvd.nist.gov/vuln/detail/CVE-2019-9506"],
    "signature_tokens": ["CVE-2019-9506"],
}


def _run_poc(plugin, vuln=None) -> dict:
    """
    CVE-2019-9506 主动探针包装（兼容旧式 exploit() 实现）。
    ...
    evidence = {
        "cve":       vuln.get("cve", "CVE-2019-9506") if vuln else "CVE-2019-9506",
...
class BTKNOBKeyLengthAuditPlugin(IVIVulnerabilityPlugin):
    ...
    meta_poc_name = 'CVE-2019-9506 BT KNOB Key Length Active Validation'
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 22_CVE_2019_9506_BT_KNOB_Key_Length_Active_Validation.py <target_mac> [bluetooth_version] [min_key_size]")
        raise SystemExit(1)
