...
def _check_wpa_supplicant_ft() -> dict:
    """Check wpa_supplicant version and FT (802.11r) support for KRACK."""
    info = {"version": "", "ft_enabled": None, "potentially_vulnerable": None}
    try:
        r = subprocess.run(
            ["wpa_supplicant", "-v"],
        ...
        if m:
            ...
            # CVE-2017-13079: FT KRACK, fixed in 2.7
    ...
    # Check if FT (802.11r) is configured
    try:
        r = subprocess.run(
            ["cat", "/etc/wpa_supplicant/wpa_supplicant.conf"],
            capture_output=True, text=True, timeout=3,
        )
        if "ft_" in r.stdout.lower() or "ieee80211r" in r.stdout.lower():
            info["ft_enabled"] = True
        else:
            info["ft_enabled"] = False
    except (FileNotFoundError, subprocess.TimeoutExpired, PermissionError):
        info["ft_config_check"] = "unavailable"
...
def _run_poc(plugin) -> dict:
    ...
    evidence = {
        "cve": "CVE-2017-13079",
        "target": target_bssid or "WPA2 AP with 802.11r FT (KRACK FT key reinstall)",
        "technique": (
            "KRACK FT handshake key reinstall: check wpa_supplicant version, "
            "verify 802.11r FT configuration, audit PTK reinstall via FT handshake"
        ),
        "reference": "https://nvd.nist.gov/vuln/detail/CVE-2017-13079",
        "vuln_detail": (
            "KRACK: PTK-TK reinstallation in the FT (Fast Transition) handshake. "
            "Affects any device using 802.11r fast BSS transition."
        ),
        "krack_type": "PTK reinstall in FT (802.11r) handshake",
    }

    wpa_info = _check_wpa_supplicant_ft()
    evidence["wpa_supplicant"] = wpa_info

    if wpa_info.get("potentially_vulnerable") is True:
        vulnerable = True
        evidence["note"] = (
            f"wpa_supplicant {wpa_info['version']} is vulnerable to KRACK FT attack. "
            "Use krackattacks-scripts/krack-ft-test.py for confirmation."
        )
    elif wpa_info.get("potentially_vulnerable") is False:
        vulnerable = False
        evidence["note"] = "wpa_supplicant version is patched for CVE-2017-13079"
    else:
        vulnerable = None
        evidence["note"] = (
            "Cannot determine FT KRACK exposure without wpa_supplicant version. "
            "Monitor mode adapter and 802.11r-capable AP required."
        )
...
class Poc57CVE201713079KRACKAuditPlugin(IVIVulnerabilityPlugin):
    meta_display_id = "POC-WIRELESS-057"
    ...
    meta_references       = ['https://nvd.nist.gov/vuln/detail/CVE-2017-13079']
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "57_WiFi_KRACK_FT_Key_Reinstall_Audit") if "VULN" in dir() else "57_WiFi_KRACK_FT_Key_Reinstall_Audit"
    ...
    _plugin = Poc57CVE201713079KRACKAuditPlugin({
