#!/usr/bin/env python3
"""CVE-2017-13084 – KRACK WPA2 key reinstallation: 4-way HS PTK reinstall in STK (FT fast transition).

Public PoC source: https://github.com/vanhoefm/krackattacks-scripts
  krack-test-client.py --fthandshake
...
VULN = {
    ...
    "component": "WPA2 key handshake (Fourway variant)",
    "type": "Key reinstallation → nonce reuse",
    "summary": "4-way HS PTK reinstall in STK (FT fast transition)",
...
def _run_poc(plugin):
    ...
    evidence = {
        ...
        "test_variant": "Fourway",
        "attack_surface": "FT 802.11r fast BSS transition replay → PTK reinstall nonce reuse",
    }
    ...
    return {
        ...
        "evidence": evidence,
        "requires_manual_review": True,
        "poc_source": "vanhoefm/krackattacks-scripts / krack-test-client.py --fthandshake",
    }
...
class Poc69Cve201713084KrackKeyReinstallAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50"
    meta_display_id = "POC-WIRELESS-069"
    meta_poc_name   = 'CVE-2017-13084 KRACK Fourway Key Reinstallation Active Validation'
    meta_cve_id     = "CVE-2017-13084"
    meta_severity   = "High"
    meta_protocol   = "wifi"
    ...
    meta_attack_surface = "FT 802.11r fast BSS transition replay → PTK reinstall nonce reuse"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "69_wpa_supplicant_hostapd_WPA2_clients_APs_802_Audit") if "VULN" in dir() else "69_wpa_supplicant_hostapd_WPA2_clients_APs_802_Audit"
    ...
    _plugin = Poc69Cve201713084KrackKeyReinstallAuditPlugin({
