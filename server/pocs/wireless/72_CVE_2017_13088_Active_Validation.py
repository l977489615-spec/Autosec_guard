#!/usr/bin/env python3
"""CVE-2017-13088 – KRACK WPA2 key reinstallation: IGTK reinstall in group key HS (MFP).
...
VULN = {
    ...
    "summary": "IGTK reinstall in group key HS (MFP)",
...
def _run_poc(plugin):
    ...
    evidence = {
        ...
        "attack_surface": "IGTK reinstall via group key handshake replay when MFP enabled",
    }
    ...
    return {
        ...
        "evidence": evidence,
        "requires_manual_review": True,
        "poc_source": "vanhoefm/krackattacks-scripts / krack-test-client.py --group --gtkinit",
    }
...
class Poc72Cve201713088KrackKeyReinstallAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50"
    meta_display_id = "POC-WIRELESS-072"
    meta_poc_name   = 'CVE-2017-13088 KRACK Grouphs Key Reinstallation Active Validation'
    meta_cve_id     = "CVE-2017-13088"
    meta_severity   = "High"
    meta_protocol   = "wifi"
    ...
    meta_attack_surface = "IGTK reinstall via group key handshake replay when MFP enabled"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "72_wpa_supplicant_hostapd_WPA2_clients_APs_802_Audit") if "VULN" in dir() else "72_wpa_supplicant_hostapd_WPA2_clients_APs_802_Audit"
    ...
    _plugin = Poc72Cve201713088KrackKeyReinstallAuditPlugin({
