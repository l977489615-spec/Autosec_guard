#!/usr/bin/env python3
"""CVE-2019-19192 – SweynTooth BLE: Sequential ATT Deadlock – STMicro BLE sequential ATT request deadlock.

Public PoC source: https://github.com/Matheus-Garbelini/sweyntooth_bluetooth_low_energy_attacks
  Script: sequential_att_deadlock.py
...
Usage:
  python sequential_att_deadlock.py /dev/ttyACM0 <target_bdaddr>
"""
...
VULN = {
    ...
    "summary": "Sequential ATT Deadlock – STMicro BLE sequential ATT request deadlock",
...
SWEYN_SCRIPT = SWEYN_REPO / "sequential_att_deadlock.py"
...
def _run_poc(plugin):
    ...
    evidence = {
        ...
        "attack_surface": "STMicroelectronics BLE SoC sequential ATT requests → deadlock/DoS",
    }
...
class Poc93CVE201919192SweynToothBleAuditPlugin(IVIVulnerabilityPlugin):
    meta_generated_source = "public_poc_exp_50_more"
    meta_display_id = "POC-WIRELESS-093"
    meta_poc_name   = 'CVE-2019-19192 SweynTooth Sequential ATT Deadlock Active Validation'
    meta_cve_id     = "CVE-2019-19192"
    meta_severity   = "High"
    ...
    meta_attack_surface = "STMicroelectronics BLE SoC sequential ATT requests → deadlock/DoS"
    is_disruptive   = True
...
if __name__ == "__main__":
    ...
    _desc = VULN.get("summary", "93_Sequential_ATT_Deadlock__STMicro_BLE_se_Audit") if "VULN" in dir() else "93_Sequential_ATT_Deadlock__STMicro_BLE_se_Audit"
    ...
    _plugin = Poc93CVE201919192SweynToothBleAuditPlugin({
